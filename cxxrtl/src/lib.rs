use cxxrtl_sys::{
    _cxxrtl_handle, cxxrtl, cxxrtl_flag_CXXRTL_DRIVEN_COMB, cxxrtl_flag_CXXRTL_DRIVEN_SYNC,
    cxxrtl_flag_CXXRTL_INPUT, cxxrtl_flag_CXXRTL_OUTPUT, cxxrtl_flag_CXXRTL_UNDRIVEN,
    cxxrtl_object,
};
...
impl<const N: u32> CxxrtlObject<N> {
    ...
    fn flags(flags: u32) -> Vec<CxxrtlFlag> {
        let all_flags = [
            ...
            // (cxxrtl_flag_CXXRTL_INOUT, CxxrtlFlag::InOut),
...
pub struct CxxrtlHandle {
    sim: cxxrtl,
    handle: *mut _cxxrtl_handle,
}

impl CxxrtlHandle {
    pub unsafe fn new(lib: &str) -> Self {
        let sim = cxxrtl::new(lib).expect("failed to load");
        let top = sim.cxxrtl_design_create();
        let handle = sim.cxxrtl_create(top);
    ...
    pub fn step(&mut self) {
        unsafe { self.sim.cxxrtl_step(self.handle) };
    }
}
...
impl Drop for CxxrtlHandle {
    fn drop(&mut self) {
        unsafe {
            self.sim.cxxrtl_destroy(self.handle);
        }
    }
}
